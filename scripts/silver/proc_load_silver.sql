...
Script Purpose:
	...
	populate 'silver' schema tables from bronze schema. 
It performs the following actions:
    - Truncates the silver tables before loading data.
	- Inserts transsformed and cleansed data from Bronze into Silver tables.
...
BEGIN TRY
	...
	SELECT 
	prd_id,
	REPLACE(SUBSTRING(prd_key, 1, 5), '-', '_') AS cat_id, -- Extract Category ID (aka Derived Column)
	...
	CAST(LEAD(prd_start_dt) OVER(PARTITION BY prd_key ORDER BY prd_start_dt) - 1 AS DATE) AS prd_end_dt -- Put end date as 1 day before the next start date. (aka Data Enrichment)
	...
	-- 3. For silver.crm_sales_details
	SET @start_time = GETDATE();
